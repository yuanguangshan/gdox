...
> Generated by [Gdox](https://github.com/yuanguangshan/gdox) on 2026-04-17 17:07:14
...
```json
{
  ...
  "scripts": {
    "build": "VERSION=$(node -p \"require('./package.json').version\") && LDFLAGS=\"-X main.versionStr=v$VERSION\" && GOOS=darwin GOARCH=amd64 go build -ldflags \"$LDFLAGS\" -o gdox-darwin-amd64 gdox.go && GOOS=darwin GOARCH=arm64 go build -ldflags \"$LDFLAGS\" -o gdox-darwin-arm64 gdox.go && GOOS=linux GOARCH=amd64 go build -ldflags \"$LDFLAGS\" -o gdox-linux-amd64 gdox.go && GOOS=linux GOARCH=arm64 go build -ldflags \"$LDFLAGS\" -o gdox-linux-arm64 gdox.go && GOOS=windows GOARCH=amd64 go build -ldflags \"$LDFLAGS\" -o gdox-win-amd64.exe gdox.go",
